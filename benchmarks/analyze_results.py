...
def analyze_results(filepath: str, output_dir: str):
    ...
    if "metrics.reasoning_tokens" in df.columns:
        ...
        if pd.notna(corr_mean) and pd.notna(incorr_mean):
            plt.text(
                0,
                df["metrics.reasoning_tokens"].max() * 0.95,
                f"Mean: {corr_mean:.1f}",
                ha="center",
                va="center",
    ...
    # 2. Rolling Accuracy by Reasoning Length.
    # Sorting by length lets us inspect whether longer reasoning tracks with better outcomes.
    if "metrics.reasoning_tokens" in df.columns:
        ...
        df_sorted["rolling_accuracy"] = (
            df_sorted["is_correct"].astype(float).rolling(window=window_size, center=True).mean()
            * 100
        )
        ...
        plt.scatter(
            df_sorted["metrics.reasoning_tokens"],
            df_sorted["is_correct"].astype(float) * 100,
            alpha=0.1,
            color="grey",
            label="Raw (0 or 1)",
        )
